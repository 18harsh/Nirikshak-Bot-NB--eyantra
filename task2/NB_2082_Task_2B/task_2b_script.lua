...
function receiveData(inInts,inFloats,inStrings,inBuffer)
    ...
    do
        ...
        for j=1,10
        do
            maze_array[i-1][j-1] = inInts[i*10-(10-j)]
        end
    end
...
function generateHorizontalWalls()
    ...
    do  
        ...
        for j=0,9
        do  
            HorArr[i][j] = createWall()
            sim.setObjectParent(HorArr[i][j],sim.getObjectHandle("Base"),false)
            sim.setObjectName(HorArr[i][j],string.format("H_WallSegment_%dx%d",i+1,j+1))
            sim.setObjectPosition(HorArr[i][j],-1,{-0.45+0.1*j,0.5-0.1*i,0.08})
        end
    end
    --print(createWall())
...
function generateVerticalWalls()
    ...
    do  
        ...
        do  
            wall = createWall()
            VertArr[i][j] = wall
            sim.setObjectParent(VertArr[i][j],sim.getObjectHandle("Base"),false)
            sim.setObjectName(VertArr[i][j],string.format("V_WallSegment_%dx%d",i+1,j+1))
            sim.setObjectOrientation(VertArr[i][j],-1,{0,0,-86.393})
            sim.setObjectPosition(VertArr[i][j],-1,{-0.5+(0.1*j),0.45-(0.1*i),0.08})
        end
    end
...
function deleteWalls()
    ...
    do  
        for j=0,10
        do  
            if VertArr[i][j]~=0 then
            sim.removeObject(VertArr[i][j])
            end
        end
    end
    ...
    do  
        for j=0,9
        do  
            if HorArr[i][j]~=0 then
            sim.removeObject(HorArr[i][j])
            end
        end
    end
    ...
    --               ADD YOUR CODE HERE
       for i=0,9 do
        for j=0,9 do
            ...
            elseif maze_array[i][j]==1 then
                    if HorArr[i][j]~=0 then
                        sim.removeObject(HorArr[i][j])
                        HorArr[i][j] = 0
                    end
                    
            elseif maze_array[i][j]==2 then
                if VertArr[i][j]~=0 then
                    sim.removeObject(VertArr[i][j])
                    VertArr[i][j]=0
                end
            
            elseif maze_array[i][j]==4 then
                if VertArr[i][j]~=0 then
                    sim.removeObject(VertArr[i][j])
                    VertArr[i][j]=0
                end
            ...
            elseif maze_array[i][j]==5 then
                if HorArr[i][j]~=0 then
                    sim.removeObject(HorArr[i][j])
                    HorArr[i][j] = 0
                end
                
            elseif maze_array[i][j]==6 then
            ...
            elseif maze_array[i][j]==9 then
                if HorArr[i][j]~=0 then
                    sim.removeObject(HorArr[i][j])
                    HorArr[i][j] = 0
                end
                
            elseif maze_array[i][j]==10 then
                if VertArr[i][j]~=0 then
                    sim.removeObject(VertArr[i][j])
                    VertArr[i][j] = 0
                end
          
            elseif maze_array[i][j]==12 then
                if VertArr[i][j]~=0 then
                    sim.removeObject(VertArr[i][j])
                    VertArr[i][j] = 0
                end
            ...
            elseif maze_array[i][j]==13 and HorArr[i][j]~=0 then
                sim.removeObject(HorArr[i][j])
                HorArr[i][j]=0
                
            elseif maze_array[i][j]==14 and VertArr[i][j]~=0 then
                    sim.removeObject(VertArr[i][j])
                    VertArr[i][j]=0
            end
        end
